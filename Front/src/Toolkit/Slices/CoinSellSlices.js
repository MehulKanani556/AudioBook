...
export const getCoinSell = createAsyncThunk(
    "getCoinSell",
    async (_, { rejectWithValue }) => {
        try {
            const response = await axios.get(`${API_URL}/getAllCoinScriptionSell`,);
...
const CoinSellSlice = createSlice({
    name: "CoinSell",
    initialState: {
        coinSell: [],
    },
    ...
    extraReducers: (builder) => {
        builder
            .addCase(getCoinSell.fulfilled, (state, action) => {
                state.coinSell = action.payload;
            })
    },
});

export default CoinSellSlice.reducer;